 ...
 *
 * Enthält `localtrack-timeline-card`: den Tages-Track einer Person auf der
 * Landkarte. Die Daten kommen ausschließlich über das WebSocket-Kommando
 * `localtrack/history` aus der Integration `localtrack`.
 *
 * Direkt darunter steckt Leaflet 1.9.4 (BSD-2-Clause) samt CSS und Marker-
 * Bildern als data:-URIs — abgegrenzt zwischen `BEGIN VENDOR` und `END VENDOR`.
 * Grund: HACS liefert bei einer Dashboard-Ressource nur diese eine Datei aus,
 ...
 *
 * Herkunft: https://unpkg.com/leaflet@1.9.4/dist/  (leaflet.js, leaflet.css,
 * images/*.png). Eingebettet statt nachgeladen, weil eine HACS-Dashboard-
 * Ressource genau *eine* Datei ausliefert — Unterordner wie `vendor/leaflet/`
 * erreichen den Browser nie und ergäben 404. Nur der Sourcemap-Verweis wurde
 * entfernt, die PNGs stecken als data:-URIs im CSS und in `LEAFLET_IMAGES`.
 * Der Lizenzkopf unten gehört zu Leaflet und bleibt erhalten.
 * ═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══ END VENDOR — Leaflet 1.9.4 ══════════════════════════════════════════ */

/* ────────────────────────────────────────────────────────────────────────────
 * localtrack-timeline-card — Google-Timeline-Nachbau für die Local-Track-Integration
 *
 * Rendert den Tages-Track einer `person.*`- oder `device_tracker.*`-Entität mit
 * Die Punkte kommen über das WebSocket-Kommando `localtrack/history` aus der
 * Integration `localtrack` — kein REST, kein Token. Leaflet steckt vollständig
 * in dieser Datei (siehe Kopf); sein CSS muss in Shadow-DOM **und**
 * Dokument-<head>, sonst fehlen Tile-/Marker-Styles (Shadow-DOM-Falle aus dem
 * Plan).
 * ──────────────────────────────────────────────────────────────────────────── */
...
const TIMELINE_CARD_SCHEMA = [
  ...
  {
    name: "height",
    selector: { number: { min: 240, max: 720, step: 10, mode: "box", unit_of_measurement: "px" } },
  },
  {
    name: "stay_radius_m",
    selector: { number: { min: 10, max: 1000, step: 10, mode: "box", unit_of_measurement: "m" } },
  },
  {
    name: "min_stay_minutes",
    selector: { number: { min: 1, max: 240, step: 5, mode: "box", unit_of_measurement: "min" } },
  },
  { name: "show_scrubber", selector: { boolean: {} } },
  { name: "reverse_geocode", selector: { boolean: {} } },
  {
    name: "max_points",
    selector: { number: { min: 100, max: 5000, step: 100, mode: "box" } },
  },
...
/**
 * Return the embedded Leaflet as a promise.
 *
 * Es wird nichts mehr nachgeladen — Leaflet steht oben in dieser Datei. Die
 * Promise-Schnittstelle bleibt, damit die Aufrufer unverändert bleiben.
 */
...
/**
 * Inject the Leaflet stylesheet into the document `<head>`, once.
 * Idempotent: a second call reuses the existing style element.
 */
function injectLeafletCssHead() {
  if (document.getElementById("localtrack-leaflet-css")) return;
  const style = document.createElement("style");
  style.id = "localtrack-leaflet-css";
  style.textContent = LEAFLET_CSS;
  document.head.appendChild(style);
}

/* ── small geometry / date helpers ──────────────────────────────────────── */
...
/**
 * Group a day's points into `stay` and `move` segments.
 *
 * Consecutive points whose running centroid stays within `stayRadiusM` are one
 * stay. A stay only counts once it has lasted `minStayMinutes` — shorter
 * gatherings are swallowed into the move that crosses them. A `move` is the
 * stretch between two stays; its length is the summed haversine distance.
 * Returns segments in chronological order; `points` carries the raw index
 * range for the map to highlight.
 */
function computeSegments(points, stayRadiusM, minStayMinutes) {
  const segments = [];
  if (!points.length) return segments;

  const closeSegment = (from, to, startIndex, endIndex) => {
    if (to - from < 30) return null; // under 30 s: not a real stop
    return {
      kind: "stay",
      start: from,
      end: to,
  ...
  const closeMove = (from, to, startIndex, endIndex) => {
    let distance = 0;
    ...
    if (distance < 25 && duration < 120) return null; // noise between two stops
    return { kind: "move", start: from, end: to, startIndex, endIndex, distance };
  };

  // Running sum for a stable centroid without re-summing on every point.
  let sumLat = 0;
  let sumLon = 0;
  let count = 0;
  let stayStart = points[0].ts;
  let stayStartIndex = 0;
  let lastIndex = 0; // last point of the current stay candidate
  let lastMoveEndIndex = -1;

  const flushStay = (endIndex) => {
    const segment = closeSegment(
      stayStart, points[endIndex].ts, stayStartIndex, endIndex
    );
    if (segment) {
      segment.lat = sumLat / count;
      segment.lon = sumLon / count;
  ...
  for (let i = 0; i < points.length; i += 1) {
    ...
    const distance = haversineMeters(centroidLat, centroidLon, point.lat, point.lon);

    if (distance <= stayRadiusM) {
      sumLat += point.lat;
      sumLon += point.lon;
      count += 1;
      lastIndex = i;
    } else {
      const stayEnd = points[lastIndex].ts;
      if (stayEnd - stayStart >= minStayMinutes * 60) {
        flushStay(lastIndex);
        lastMoveEndIndex = lastIndex;
      }
      // Start a new candidate at this point.
      sumLat = point.lat;
      sumLon = point.lon;
      count = 1;
      stayStart = point.ts;
  ...
  // Trailing candidate.
  if (count > 0 && points[lastIndex].ts - stayStart >= minStayMinutes * 60) {
    flushStay(lastIndex);
  }

  // Insert the moves between consecutive stays.
  const result = [];
  const stays = segments;
  for (let i = 0; i < stays.length; i += 1) {
    const stay = stays[i];
    const prevEnd = i === 0 ? 0 : stays[i - 1].endIndex;
    const prevStart = i === 0 ? null : stays[i - 1];
    if (prevStart === null) {
      // Before the first stay there may be an initial move from the first point.
      if (stay.startIndex > 0) {
        const move = closeMove(
          points[0].ts, stay.start, 0, stay.startIndex
        );
        if (move) result.push(move);
      }
    } else if (stay.startIndex > prevEnd + 1) {
      const move = closeMove(
        points[prevEnd].ts, stay.start, prevEnd, stay.startIndex
      );
      if (move) result.push(move);
    }
    result.push(stay);
  }
  // After the last stay, a move to the end of the day.
  const last = stays[stays.length - 1];
  if (last && last.endIndex < points.length - 1) {
    const move = closeMove(
      last.end, points[points.length - 1].ts, last.endIndex, points.length - 1
    );
    if (move) result.push(move);
  }
...
/* ── zone matching + reverse geocoding ──────────────────────────────────── */

function findZone(hass, lat, lon) {
  let best = null;
  for (const [entityId, state] of Object.entries(hass.states || {})) {
    if (!entityId.startsWith("zone.")) continue;
    const zoneLat = state.attributes.latitude;
    const zoneLon = state.attributes.longitude;
    const radius = state.attributes.radius;
    if (zoneLat == null || zoneLon == null || radius == null) continue;
    const distance = haversineMeters(zoneLat, zoneLon, lat, lon);
    if (distance <= radius) {
      if (!best || radius < best.radius) best = { name: state.attributes.friendly_name || entityId, radius };
    }
  }
...
/** Reverse-geocode one coordinate via Nominatim, cached and rate-limited. */
function reverseGeocode(lat, lon) {
  const key = `${lat.toFixed(5)},${lon.toFixed(5)}`;
  const cached = reverseGeocode._cache.get(key);
    ...
    fetch(
      ...
      .then((data) => {
        ...
        const name = address.road || address.neighbourhood || address.suburb ||
          address.village || address.town || address.city || data.name || "";
        reverseGeocode._cache.set(key, name);
        if (reverseGeocode._cache.size > 500) {
          const firstKey = reverseGeocode._cache.keys().next().value;
          reverseGeocode._cache.delete(firstKey);
        }
        return name;
      })
      .catch(() => {
        reverseGeocode._cache.set(key, "");
        return "";
      });

  geocodeChain = geocodeChain.then(run, run);
  return geocodeChain;
}
reverseGeocode._cache = new Map();
...
class LocaltrackTimelineCard extends HTMLElement {
  ...
  static getStubConfig(hass) {
    ...
    return { type: "custom:localtrack-timeline-card", entity: entity || "" };
  }
  ...
  set hass(hass) {
    ...
    if (this._day === today && before && now && before.last_updated !== now.last_updated) {
      // Only live-refresh while looking at today; past days do not change.
  ...
  connectedCallback() {
    if (this._config && this._hass && !this._map) this._loadDay();
    if (this._mapEl && !this._resizeObserver) {
  ...
  getCardSize() {
    return Math.max(6, Math.round((this._config?.height || 320) / 50));
  }
  ...
  _build() {
    ...
    if (!this._day) this._day = this._todayString();
    this._dateEl.value = this._day;
    this._dateEl.addEventListener("change", () => {
      this._day = this._dateEl.value;
      this._loadDay();
    });
    this._scrubberEl.addEventListener("input", () => this._renderScrubber());
    this._segmentsEl.addEventListener("click", (event) => {
      const row = event.target.closest(".segment");
      if (!row || !this._map) return;
  ...
  async _loadDay() {
    ...
    try {
      ...
      const result = await this._hass.callWS({
        ...
        max_points: Number(this._config.max_points) || 2000,
      });
      ...
      this._segments = computeSegments(
        ...
        Number(this._config.min_stay_minutes) || 10
      );
      this._renderTrack();
  ...
  async _ensureMap() {
    injectLeafletCssHead();
    if (!this._leaflet) this._leaflet = await loadLeaflet();
    ...
    if (!this._map) {
      ...
      L.Icon.Default.mergeOptions({
        iconUrl: LEAFLET_IMAGES.markerIcon,
        iconRetinaUrl: LEAFLET_IMAGES.markerIcon2x,
        shadowUrl: LEAFLET_IMAGES.markerShadow,
      });

      this._map = L.map(this._mapEl, { zoomControl: true, attributionControl: true });
      const tileUrl = (this._config.tile_url || "").trim() ||
  ...
  _renderTrack() {
    ...
    if (this._markers) this._markers.forEach((marker) => this._map.removeLayer(marker));
    this._markers = [];
    this._currentMarker = null;
    if (this._markerLayer) this._markerLayer.clearLayers();
    this._markerLayer = L.layerGroup();
    const latLngs = this._points.map((point) => [point.lat, point.lon]);
    if (latLngs.length === 0) {
    ...
    // One numbered pin per stay, numbered in visit order.
    let stayNumber = 0;
    ...
    // Attach the finished group to the map last: Leaflet needs `_map` set on a
    // circle marker before its first `_update`, which does not happen reliably
    // when the marker is added to a group that is already on the map.
    this._markerLayer.addTo(this._map);
  ...
  _renderScrubber() {
    if (!this._points.length || !this._map) return;
    const L = this._leaflet;
    const max = this._points.length - 1;
    this._scrubberEl.max = String(max);
    const index = Math.min(Number(this._scrubberEl.value), max);
    ...
    this._scrubLabelEl.textContent = `${formatClock(date, this._hass?.locale?.language || "de")} · ${zone}`;
    if (this._hass?.locale?.language) this._scrubLabelEl.style.fontFamily = "";
    if (!this._map.getBounds().contains([point.lat, point.lon])) {
      this._map.panTo([point.lat, point.lon]);
    }
  }
  ...
  async _renderSegments() {
    ...
    for (let i = 0; i < this._segments.length; i += 1) {
      ...
      const startDate = new Date(segment.start * 1000);
      const endDate = new Date(segment.end * 1000);
      const times = `${formatClock(startDate, locale)}–${formatClock(endDate, locale)}`;
      let label;
      let meta;
    ...
    this._segmentsEl.innerHTML = rows.join("");
    // Reverse geocoding filled in labels above; also refresh the scrubber label.
    if (this._points.length) this._renderScrubber();
  }
...
customElements.define("localtrack-timeline-card", LocaltrackTimelineCard);
...
class LocaltrackMapCard extends HTMLElement {
  static getConfigElement() {
    return document.createElement("localtrack-map-card-editor");
  }
  ...
  setConfig(config) {
    if (!config) throw new Error("localtrack-map-card: Konfiguration fehlt");
  ...
  _sucheKarte() {
    ...
    const versuch = () => {
      ...
      if (!this._gewarnt) {
        this._gewarnt = true;
        console.warn(
          "localtrack-map-card: `ha-map.leafletMap` nicht gefunden — die Kacheln "
  ...
  _applyTiles() {
    ...
    if (raster) {
      ...
      // Kein Raster da (Vektorebene): eine eigene anlegen. Das ist der
      // riskantere Zweig, deshalb steht er hinten.
      try {
        map.eachLayer((layer) => {
          if (layer.getPane && layer.getPane() === map.getPane("tilePane")) {
            map.removeLayer(layer);
          }
        });
        const neu = LEAFLET.tileLayer(url, {
        ...
        console.warn("localtrack-map-card: eigene Kachelebene fehlgeschlagen", error);
        return false;
      }
    }

    // HAs Dunkelmodus-Filter abschalten: echte dunkle Kacheln brauchen keine
    // Invertierung, und beides zusammen ergibt Matsch.
    if (this._inner) {
...
class LocaltrackMapCardEditor extends HTMLElement {
  setConfig(config) {
    this._config = config || {};
    this._render();
  }
...
customElements.define("localtrack-map-card", LocaltrackMapCard);
customElements.define("localtrack-map-card-editor", LocaltrackMapCardEditor);

window.customCards.push({
  type: "localtrack-map-card",
  name: "Local Track Landkarte",
